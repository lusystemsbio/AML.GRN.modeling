selectTOPtfs.byMethod <- function(TFs.combined, targetDB, top.TFs.count){ 
   ...
   #tfs.df=TFs.combined[['Netact']][,c(1, 4)] # select columns: tfs and pvalue  
   #tfs.df <-  tfs.df[order(tfs.df[,2], decreasing = FALSE), ] # order them by pvalue - smallest to largest
   
   tfs_tmp.df <- TFs.combined[['Netact']]
   tfs.df <-  tfs_tmp.df[order(tfs_tmp.df[,6], decreasing = FALSE), ] # order them by qvalue - smallest to largest
   tfs.df <- tfs.df[,c(1, 4)] # extract tfs and pvalues 
   ...
   #tfs.df=TFs.combined[['MARINa']][,c(1, 3)] # select columns: tfs and pvalue
   #tfs.df <-  tfs.df[order(tfs.df[,2], decreasing = FALSE), ] # order them by FDR - smallest to largest 
   
   tfs_tmp.df <- TFs.combined[['MARINa']] 
   tfs.df <-   tfs_tmp.df[order( tfs_tmp.df[, 4], decreasing = FALSE), ] # order them by FDR - smallest to largest 
   tfs.df <- tfs.df[,c(1, 3)] # extract tfs and pvalues 
...
plotStat.comb <- function(stats.df, colors, figname, ylimit, xlimit){
...
plotStat <- function(stats.df, colors, figname, ylimit, xlimit){
   colors <- c('black', 'magenta','blue', 'red') 
   names(colors) <- c("TFs", "ACTUAL", "CTRL", "TRTMT")  
   WIDTH <- 10
   HEIGHT <- 8 
