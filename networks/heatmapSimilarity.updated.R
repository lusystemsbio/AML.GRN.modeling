...
sracipeHeatmapSimilarity = function(
 ...
 if(is.null(colnames(dataReference))){
  ...
  # Cluster samples (using all genes) if cluster id of 
  # the samples are not supplied
  # ----------------------------------------------------------
  ...
  # Find variances within each cluster of reference data
  ...
  # Find the minimum distance between a model and the closest sample 
  # of the cluster which the model belongs to: simulatedClusterVar
  #----------------------------------------------------------------
  # calculate the correlations between each model and each sample: 
  ...
  # Calculate pvalue for each model being matched to each of the clusters  
  # Based on the calculated pvalue, designate each model belonging to any of 
  # the clusters or non-match (0th cluster)
  #-------------------------------------------------------------------------
  if (method == "variance") {
  ...
  # Calculate distribution of the models belonging to each of the clusters
...
#     print(dataRefSamples)
 ...
 #   print(similarity$dataSimulation)
  ...
  # Calculate average distance between the RACIPE models and the reference samples 
  #------------------------------------------------------------------------------- 
