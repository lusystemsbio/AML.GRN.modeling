...
sampleSizes.new <- c('1K', '2K', '3K', '4K', '5K', '6K', '7K', '8K', '9K', '10K') 
names(sampleSizes.new) <- sampleSizes
sampleSizes.new['05K']  
as.character(sampleSizes.new['05K'])   
...
# Obtain metrics of top 5 circuits across samples for each  
# sample size level
#---------------------------------------------------------
...
for(sampleSize in sampleSizes){ 
   print(sampleSize)
   fname.input <- paste("./samples/comb.metrics.SSIZE.", sampleSize, '/results/', 
   ...
   # top.circuit_id.list[[sampleSize]] <- rownames(metric.top_circuits)
   # metric.top_circuits.list[[sampleSize]] <- metric.top_circuits   
   
   top.circuit_id.list[[sampleSizes.new[sampleSize]]] <- rownames(metric.top_circuits)
   metric.top_circuits.list[[sampleSizes.new[sampleSize]]] <- metric.top_circuits   
}
...
WIDTH <- 10
HEIGHT <- 6
fname.out <- paste(outdir, 'dotplot_scores.bySampleSize-', 
                   NO_TOP_CIRCUITS, 'TOP.circuits' ,'.pdf', 
                   sep = '')
ggsave(filename = fname.out, g, width = WIDTH, height = HEIGHT)
...
fname.out <- paste(outdir, 'dotplot_log10Pval.bySampleSize-', 
                   NO_TOP_CIRCUITS, 'TOP.circuits' ,'.pdf', 
                   sep = '')
...
fname.out <- paste(outdir, 'dotplots_scores_and_pvalues-', 
                   NO_TOP_CIRCUITS, 'TOP.circuits' ,'.pdf', 
                   sep = '')
