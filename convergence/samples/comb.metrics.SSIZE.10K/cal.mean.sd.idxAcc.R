...
metric.comb$S1 <- circuit_metrics.ordered$idxAccuracy
...
for(sample_name in names(circuit_metrics.sim)[2:NO_SAMPLES]){  
   ...
   metric.comb[,sample_name] <- circuit_metrics.ordered$idxAccuracy
}
...
write.csv(format(metric.comb.sorted, digits=3), file = paste0(datadir, 'metric.idxAcc.sorted.csv'), 
          row.names = T, quote = F)
