...
sracipeHeatmapSimilarity = function(
 ...
 if(is.null(colnames(dataReference))){
  ...
  # 2. Calculate correlations between samples 
  refCor <- cor((dataReference), method = corMethod) 
  
  # 3. Cluster samples (using all genes) if cluster id of the samples are not supplied
  # -------------------------------------------------------------------------------
  ...
  # 4. Find variances within each cluster of reference data
  # (Calculate the minimum of the distances between sample pairs 
  # in each cluster) - refClusterVar  
  #--------------------------------------
  # Use only the selected genes for comparison. on the other hand, 
  ...
  # obtain the minimum pairwise distance in each cluster:  
  ...
  # 5. Find the minimum distance between a model and the closest sample 
  # of the cluster which the model belongs to: simulatedClusterVar
  #----------------------------------------------------------------
  # 5a. calculate the correlations between each model and each sample: 
  # rows: models, columns: samples
  ...
  # 5b. calculate two values:  
  # (1) the distance between a model and the closest sample in each cluster - simulatedClusterVar
  # (2) the mean of the distances between a model and all the samples in each cluster - mean_dist.model2cluster
  ...
  # 6. Classify each model to one of the two clusters or to the third cluster (Null) 
  # a. For each model, build a null distribution of correlations between the gene-permuted models and a cluster
  # b. Using this null distribution, calculate P value for the model’s membership to a cluster. There are two clusters and hence two P values. 
  # c. Based on the two calculated P values, designate each model belonging to any of the two clusters or 
  #    non-match (0th cluster) – if both P value is greater than P value threshold, assign the model to Null group, 
  #    otherwise assign the model to the group with smaller P value.
  #-------------------------------------------------------------------------
  if (method == "variance") {
  ...
  # invoke the hS.diagnostic code here
  #----------------------------------
  
  # 7. Calculate distribution of the models belonging to each of the clusters
  # (including the 0th cluster). 
  # Do the same for the samples.
  # Subsequently, these distributions will be used to calculate KL distance 
  #-------------------------------------------------------------------------
  similarity <- list()
...
#     print(dataRefSamples)
 ...
 #   print(similarity$dataSimulation)
  ...
  # 9. Calculate average distance between the RACIPE models and the reference samples 
  #------------------------------------------------------------------------------- 
  # need to consider two variables: 
  #   (1) mean_dist.model2cluster - this gives the distance between a model and each of the cluster 
  #       (here, distance is the mean of the distances with the samples in the cluster) 
  #   OR, simulatedClusterVar - this gives the distance between a model and each of the cluster 
  #       (with the closest sample in the cluster) - an alternate way
  #   (2) simulated.cluster - this gives which cluster the model belongs to
  mean.model_dist <- c(rep(0, nClusters))
  for(cluster_no in 1:nClusters){
     print(cluster_no) 
     #dist.models <- simulatedClusterVar[simulated.cluster==cluster_no, ] 
     dist.models <- mean_dist.model2cluster[simulated.cluster==cluster_no, ] 
     
     mean.model_dist[cluster_no] <- mean(dist.models[,cluster_no]) 
     #break()
  } 
  avg_dist.model_ref <- mean(mean.model_dist) 
  
  # normalize the average distance by the number of genes in the topology:
  similarity$AvgDist <- avg_dist.model_ref/sqrt(length(commonGenes)) 
  
  # 10. return similarity object
  return(similarity)
}
...
#' @return An array of dimension n.models by nClusters by permutations.
#'
.ModelPvalue <- function(dataSimulation, dataReference, clusterCut, 
                         permutations, refClusterVar, corMethod, 
                         simulatedClusterVar){ 
  nClusters <- length(unique(clusterCut))
  nModelsKO <- dim(dataSimulation)[2] 
  ...
  # 
  for(j in seq_len(nClusters)){
    # calculate distances between each random model in randomModels and 
